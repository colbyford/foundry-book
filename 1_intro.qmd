...
Microsoft Foundry delivers a multiple capabilities to manage enterprise agentic solutions at-scale and from end-to-end. As shown in @fig-4C72 below, these include:
...
Microsoft Foundry utilizes two services in Azure: the Foundry Resource and the Foundry Project. See @fig-3498 below for a visual of how these two services are relate to one another.
...
At the top of the Azure Portal, you may see some of the previous services that you've accessed. Above this line of icons, click inside the search box as shown in Figure @fig-693E.

![Azure Portal - Search Box](figures/in/ss_portal_001_ann.png){#fig-693E}


In the search box, simply type "Microsoft Foundry". In the search results, locate the **Microsoft Foundry** option under the **Services** section and click on it as shown in Figure @fig-992A.
...
Azure will now run a validation check to make sure it will be able to deploy the requested items. This may take a few seconds. If it fails, simply go back and fix the items based on the failure message. Once the validation passes, click the **Create** button at the bottom of the screen as shown in Figure @fig-5918.

![Create a Foundry Resource - Review + Create tab](figures/in/ss_portal_006_ann.png){#fig-5918}


Azure will then show a Deployment Overview page, as seen in @fig-CC82, that lists the items that are being deployed along with their statuses. This may take a couple minutes for all the resources to be provisioned.
...
Locate the Foundry Resource and click on it. See @fig-3954 below.

![Azure Portal - Resource Group of Foundry-related Resources](figures/in/ss_portal_008_ann.png){#fig-3954}


On the Foundry Resource page, this is where you can manage various components of the service. For example, you can control who has access using RBAC, limit network access, view and rotate access keys, view some metrics, and more. For now, simply click the **Go to Foundry portal** on the **Overview** page, as show in @fig-A959.
...
The Microsoft Foundry portal provides an intuitive interface organized into several key sections, accessible via the portal header, shown in @fig-B78E below.
...
Once you get logged in, the CLI will let you pick the Subscription under which it will operate. Simply type the number of the Subscription you wish to use and press Enter. For example, my list of subscriptions looks is shown in @fig-360B below, and I would type `5` to select the "Microsoft Azure Sponsorship" subscription.
