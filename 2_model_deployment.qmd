...
::: {.callout-note}
For more information on MCP, visit the docmentation at: [modelcontextprotocol.io/docs/](https://modelcontextprotocol.io/docs/).
...
Navigate to the Azure Portal ([portal.azure.com](https://portal.azure.com)) and locate the Cosmos DB account. On the side panel, click on **Settings** dropdown and then **Keys**.

![The Cosmos DB screen in the Azure Portal](figures/ch02/ch02_ss_cosmos_001_ann.png)

This screen will show the connection information for your Cosmos DB account. Copy and paste your **URI** and **Primary Key** (after clicking the **eye** button to unhide it) to a notepad. We'll need these things in just a moment. (Keep this tab open! We'll need to come back here soon.)
...
On the **Connect the Azure Cosmos DB tool** screen, paste in the URI you copied from before to the **Remote MCP Server endpoint** box. Paste the key you copied into the **Credential** box and name the credential "key1". I also replaced the default name in the **Name** box as this helps to distinguish between multiple servers of the same type later on. When you're done filling in the values, click the **Connect** button.
...
4. Create a database named "RealEstateCatalog"
5. Create a container named "Properties" with partition key `/state`
