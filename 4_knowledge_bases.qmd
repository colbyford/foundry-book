...
- **Query understanding:** When a user asks complex, conversational, or vague questions with or without a specific context, traditional keyword search fails. Then,when queries don't match document terminology. So, RAG must understand intent, not just match keywords.
...
- **Token constraints:** LLMs have limited context windows (meaning they can only accept a limited number of tokens as input). Your retrieval system must not being too wordy in its responses. So, it must only return highly relevant, concise results instead of full document dumps.
...
The agent should be able to pull relevant information any of the files that you uploaded and indexed. I know that there was a slide with forward-looking statements about Microsoft's outlook for the next quarter. See @fig-257F for an example of what my Robo CFO Agent's response was.
