...
This will submit the fine-tuning job to Foundry, as shown in @fig-5DD6. You'll be prompted to install the CLI extension if it's not already installed and give the local project a name. This can be whatever you like, but I named mine `medical-assistant-full`.

![Initializing a fine-tuning job from the CLI](figures/ft/ss_cli_001_clean.png){#fig-5DD6}

First, the CLI will upload the two `.jsonl` files, and shown in @fig-19D9, and then kick off the fine-tuning job.

![Uploading training data and starting the fine-tuning job](figures/ft/ss_cli_002_clean.png){#fig-19D9}


You can monitor its status and view details using the `list` and `show` CLI commands. See @fig-E5DD.

![Listing fine-tuning jobs and showing job details](figures/ft/ss_cli_003_clean.png){#fig-E5DD}
