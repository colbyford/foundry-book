# Agent Management and Governance {#sec-management}
...
You'll notice that there are a couple Guardrails that are pre-defined for you, such as `Microsoft.DefaultV2`. These are Microsoft-created baseline Guardrails that you can assign directly to your agents and model deployments, or use as a starting point to customize your own.

Click on the *Create* button to the top-right corner of the Guardrails list to act like you're creating a new one. This will bring you to the *Create guardrail controls* page, where you can select from a list of risks to define the controls for your new guardrail, as shown in @fig-2829. Click the dropdown arrow under *Risks* and you'll see the full list of risks that Foundry's guardrail system can detect. 
...
| Protected Material | Copyrighted text or code in outputs |
...
For some Risks, you can also set the severity level (low, medium, and high blocking), which will determine how strict the detection should be. Setting this to a higher severity level results in stronger filtering and intervention. For example, you might want to block high-severity hate speech but only annotate low-severity profanity.
...
::: {.callout-important}
Note that agent-assigned Guardrails override model Guardrails. When you assign a custom guardrail directly to an agent, that Guardrail fully overrides the Guardrail of the underlying model deployment. It does not merge with it.
...
You can click into any of these assets to see detailed information about its configuration, performance, evaluation results, and guardrail annotations. For example, click on your `python-coder-agent` that you configured as part of the Workflows chapter (or any other Agents), which will take you to the *Monitor* tab of the Agent, as shown in @fig-4282. This shows the estimated cost and total token usage for the filtered time range at the top. Plus, if you scroll down, you'll see several operational metrics such as agent runs, tool calls, error rates, and token usage over time. You can also see evaluation results for this agent, such as the Task Adherence score, which is a measure of how well the agent is sticking to its defined task. 

![Viewing the Monitor tab of an Agent](figures/am/ss_controlplane_002_clean.png){#fig-4282}
...
Limits apply at the project level. So, each project can have independent TPM and quota settings, which makes this mechanism well-suited for:
...
Every agent created in Foundry is automatically issued a Microsoft Entra Agent ID at build time. This is not a service account or a shared credential. It is a dedicated, purpose-built identity type in Microsoft Entra ID, represented as a service principal with its own lifecycle and permissions.
...
The Foundry Control Plane handles operational governance at the fleet level: it surfaces every agent, model, and tool in a single inventory; enforces Subscription-wide or Resource Group-wide guardrail policies; tracks costs and performance across projects; integrates with Entra ID for durable agent identity; and connects with Defender and Purview for security and compliance.
